...
# 预处理，返回 X_train, y_train, X_test, y_test
def preprocess():
...
class KNNClassifier():
    ...
    def predict(self, X_test):
        ...
        for i in range(N):
            #closest_y = fclosest(self.y_train, dists[i], k)
            index = dists[i].argsort()
            index = index[:self.k]
            closest_y = self.y_train[index]

            #values, counts = fcount(closest_y)
            values, counts = np.unique(closest_y, return_counts=True)
            
            #y_pred[i] = values[argmax(counts)]
...
if __name__ == '__main__':
    ...
    acy = accuracy(y_pred, y_test)
    print("accurary: %f" % (acy))
